...
{
    /// <summary>
    /// 강사용 데모 씬 2개(1부 통합 기초 + 2부 클리커)를 한 번에 생성·갱신한다.
    /// 메뉴: Tools > Reactive Study > Create All Scenes
    ///
    /// 각 씬은 같은 경로에 덮어써 갱신되며, SceneCreator 가 정본이다.
    ...
    public static class AllScenesCreator
    {
        ...
        {
            ("R3 기초 통합(1부)", () => ReactiveBasicsSceneCreator.BuildScene()),
            ("리액티브 클리커(2부)", () => ClickerGameSceneCreator.BuildScene()),
        };
        ...
        public static void CreateAllScenes()
        {
            if (
                !EditorUtility.DisplayDialog(
                    "데모 씬 전체 생성",
                    "강사용 데모 씬 2개(1부 통합 기초 + 2부 클리커)를 생성·갱신합니다.\n"
                        + "기존 씬은 같은 경로에 덮어써집니다.\n"
        ...
        public static List<string> BuildAllScenes()
        {
            ...
            // 시작 씬(1부 통합 기초)을 열어 둔다.
            if (System.IO.File.Exists(ReactiveBasicsSceneCreator.ScenePath))
                EditorSceneManager.OpenScene(ReactiveBasicsSceneCreator.ScenePath, OpenSceneMode.Single);
